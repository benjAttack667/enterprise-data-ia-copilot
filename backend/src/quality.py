...
def _normalise_blanks(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in result.select_dtypes(include=["object", "string"]).columns:
        blank_mask = result[column].notna() & result[column].astype(str).str.strip().eq("")
...
def audit_data_quality(dataframe: pd.DataFrame) -> dict[str, Any]:
    ...
    for column in df.select_dtypes(include="number").columns:
        outliers = _outlier_mask(df[column]).fillna(False)
        numeric = pd.to_numeric(df[column], errors="coerce")
        invalid = (numeric.eq(np.inf) | numeric.eq(-np.inf)).fillna(False)
        outlier_masks[str(column)] = outliers
        invalid_masks[str(column)] = invalid
    ...
    penalty = (
        ...
        + min(mixed_fraction + invalid_fraction, 1.0) * 10
    )
